...
while reader.has_next
  ...
  if type == XMLStreamConstants::START_ELEMENT
    ...
    when 'revision'
      ...
      p.add(*jbytes("revision", "author", document['username'])) if document['username']
      p.add(*jbytes("revision", "comment", document['comment'])) if document['comment']
      table.put(p)
      
      count += 1
      table.flushCommits() if count % 10 == 0
      if count % 500 == 0
        puts "#{count} записей вставлено (#{document['title']})"
      end
    end
  end
end
